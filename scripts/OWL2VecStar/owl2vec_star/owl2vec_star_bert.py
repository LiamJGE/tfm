...
from tfm.scripts.OWL2VecStar.owl2vec_star.lib.Onto_Projection_bert import Reasoner, OntologyProjection
...
class callback(CallbackAny2Vec):
    ...
    def on_epoch_end(self, model):
        ...
        print('Loss after epoch {}: {}'.format(self.epoch, loss - self.prev_loss))
        self.wandb.log({"loss": loss - self.prev_loss})
        self.prev_loss = loss
        self.epoch += 1
...
'''
def extract_owl2vec_model(ontology_file, config_file, uri_doc, lit_doc, mix_doc, wandb):
    ...
    model_ = __perform_ontology_embedding(config, wandb)
...
'''
def __perform_ontology_embedding(config, wandb):
    ...
    def label_item(item):
        ...
        elif item.startswith('http://www.w3.org') and len(item.split('#')) > 1:
    ...
    if 'pre_train_model' not in config['MODEL'] or not os.path.exists(config['MODEL']['pre_train_model']):
        ...
        def preprocess_function(examples):
            return tokenizer([" ".join(x) for x in examples["text"]], return_tensors='tf', padding=True, truncation=True)
        
        # Load pre-trained BERT tokenizer
        df_sen = pd.DataFrame(data=[(' ').join(sen) for sen in all_doc], columns=['text'])

        sentences = datasets.Dataset.from_pandas(df_sen)
        # Tokenize input texts for MLM
        tokenized_inputs = sentences.map(preprocess_function, batched=True)
        ...
        model_ = TFBertForMaskedLM.from_pretrained('bert-base-uncased')

        # # Prepare labels for MLM
        # labels = tf.identity(input_ids)

        # # Masked language modeling objective
        # mlm_labels = -100 * tf.ones(input_ids.shape, dtype=tf.int32)  # -100 is used to ignore loss on non-masked tokens
        # mlm_labels = tf.where(tf.equal(input_ids, tokenizer.mask_token_id), labels, mlm_labels)

        # # Compile the model for MLM
        ...
        # model_.fit(inputs, mlm_labels, epochs=2)
        optimizer = AdamWeightDecay(learning_rate=2e-5, weight_decay_rate=0.01)

        tf_train_set = model_.prepare_tf_dataset(
            tokenized_inputs,
            shuffle=True,
            batch_size=16,
        ...
        tf_test_set = model_.prepare_tf_dataset(
            tokenized_inputs,
            shuffle=False,
            batch_size=16,
            collate_fn=data_collator,
        ...
        model_.fit(x=tf_train_set, validation_data=tf_test_set, epochs=3)
        # Save the trained model
        model_.save_pretrained("bert_mlm_model")

        # Test the model
        masked_output = model_.predict(inputs)
        logging.info('Model output')
        logging.info(masked_output)

        # model_ = gensim.models.Word2Vec(all_doc, vector_size=int(config['MODEL']['embed_size']),
        #                                 window=int(config['MODEL']['window']),
        #                                 workers=multiprocessing.cpu_count(),
        #                                 sg=int(config['MODEL']['sg']), epochs=int(config['MODEL']['iteration']),
        #                                 negative=int(config['MODEL']['negative']),
        #                                 min_count=int(config['MODEL']['min_count']), seed=int(config['MODEL']['seed']),
        #                                 compute_loss=True, callbacks=[callback(wandb)])
    else:
