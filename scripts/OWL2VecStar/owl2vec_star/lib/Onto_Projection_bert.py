...
class OntologyProjection(object):
    ...
    def extractProjection(self):
        ...
        if self.include_literals:
            ...
            logging.info("Adding literals")
            ################## CUSTOM PART START ##################
            properties = self.onto.queryGraph(self.getQueryForUniquePropertiesWithNumericObject())

            # logging.info('Numeric properties: ' + str(properties))

            for prop in properties:
                self.__addTriple__(prop[0], RDFS.range, XSD.string)
                # logging.info('Property: ' + str(prop[0]))
                results = self.onto.queryGraph(self.getQueryForTriplesWithSpecificProperty(property_uri=prop[0]))
                ...
                # Calculate quantiles
                    ...
                    df.loc[(df['Value'] >= q.values[1]) & df['Value'] < q.values[2], 'Value_cat'] = 'medium' 
                    df.loc[(df['Value'] >= q.values[2]) & df['Value'] < q.values[3], 'Value_cat'] = 'high' 
                    df.loc[(df['Value'] >= q.values[3]), 'Value_cat'] = 'very high' 

                    for _, row in df.iterrows():
                        try:
                            self.__addTriple__(URIRef(row['URI']), URIRef(row['Property']), Literal(row['Value_cat']))
                        except AttributeError:

                            pass
                except Exception:
                    logging.info('AAAAAH FUCK')
                #     logging.info(df['Value'].value_counts())
                #     logging.info(df['Value'].isna().sum())
                #     # pass

            
            # Adding Literals To Graph
            ...
            for prop in properties:
                ...
                try:
                    ...
                    logging.info('')
                    # logging.info('NOOOO FUCK')
                    # logging.info(prop)
                    # logging.info(df)
            
            
            # results = self.onto.queryGraph(self.getQueryForTriplesWithLiteralNonNumericObject())
            # # logging.info('Non numeric: ' +  str(results))
            # for row in results:
            #     try:
            #         obj_value = float(row[2])
            #     except:
            #             try:
            #                 self.__addTriple__(row[0], row[1], row[2])
            #             except AttributeError:
            #                 pass
            ##################  CUSTOM PART END  ##################
    ...
    # CUSTOM FUNCTION TO ADD LITERALS
    def getQueryForTriplesWithLiteralNonNumericObject(self):
        return """SELECT ?s ?p ?o WHERE {
            ?s ?p ?o .
            FILTER (isLiteral(?o) && !isNumeric(?o))
        }"""

    def getQueryForUniquePropertiesWithNumericObject(self):
        return """SELECT DISTINCT ?p WHERE {
            ?s ?p ?o .
            FILTER (isNumeric(?o))
        }"""

    def getQueryForUniquePropertiesWithNonNumericObject(self):
        return """SELECT DISTINCT ?p WHERE {
            ?s ?p ?o .
            FILTER (isLiteral(?o) && !isNumeric(?o))
        }"""

    def getQueryForTriplesWithSpecificProperty(self, property_uri):
        return """SELECT ?s ?p ?o WHERE {{
            ?s <{prop}> ?o .
            FILTER (isLiteral(?o))
        }}""".format(prop=property_uri)
